...
# TODO: Pass args.batch_size properly to data loaders. At the end of training phase of each epoch,
#  log the training time of passed epoch to TensorBoard to the "Training_time/train" scalar set.

# co se po nás chce aby když máme nějak velikou batch size tak aby se správně poslal tam kam má a
# zjistit jak je veliká a jakou má přesnoust a čas, všechno zalogovat
...
def main(args):
    ...
    # Load data
    mnist = MNIST()
    train_loader = DataLoader(mnist.train, batch_size=args.batch_size, shuffle=True)    # tady se upravili ty batch size
    dev_loader = DataLoader(mnist.dev, batch_size=args.batch_size, shuffle=False)       # datasety mužou být tak velké že se nevejdou do paměti a proto se porcují na batche
